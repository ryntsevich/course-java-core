...
class Task03 {
    ...
    static double[] getAverageMarks(int[][] marks) {
        double[] arraySum = new double[marks.length];
        for (int i = 0; i < marks.length; i++) {
            int sumMarks = 0;
            for (int j = 0; j < marks[i].length; j++) {
                sumMarks += marks[i][j];
            }
            arraySum[i] = BigDecimal.valueOf((double) sumMarks / marks[i].length).setScale(2, RoundingMode.HALF_UP).doubleValue();
        }
        return arraySum;
    }
    ...
    static int[] getMaxMarks(int[][] marks) {
        int[] resultArrayWithMaxMarks = new int[marks.length];
        for (int i = 0; i < marks.length; i++) {
            ...
            resultArrayWithMaxMarks[i] = maxMark;
        }
        return resultArrayWithMaxMarks;
    }
}
